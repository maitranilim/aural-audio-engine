...
const HOUSE_CORNERS = ["Tech House", "Deep House", "Tropical House", "French House", "Progressive House"] as const;

const WORKED = [
  { title: "Lean On", artist: "Major Lazer", genre: "EDM", sub: "Tropical House", micro: "Moombahton" },
  { title: "Strobe", artist: "deadmau5", genre: "EDM", sub: "Progressive House", micro: "Melodic House" },
  { title: "HUMBLE.", artist: "Kendrick Lamar", genre: "Hip-Hop", sub: "West Coast Hip-Hop", micro: "West Coast Trap" },
  { title: "Take Five", artist: "Dave Brubeck", genre: "Jazz", sub: "Cool Jazz", micro: "West Coast Jazz" },
  { title: "Midnight City", artist: "M83", genre: "Pop", sub: "Synthpop", micro: "Synthwave" },
  { title: "Around the World", artist: "Daft Punk", genre: "EDM", sub: "House", micro: "French House" },
] as const;

function useChapterBeat(length: number) {
  ...
  useGSAP(
    () => {
      ...
      const trigger = ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          const next = Math.min(length - 1, Math.floor(self.progress * length));
          setStep(next);
        },
      });
      return () => trigger.kill();
...
function Rail({
  ...
  return (
    <aside className="chapter-rail">
      ...
      <ol className="mt-6 flex gap-2 md:mt-10 md:flex-col md:gap-1">
        {beats.map((b, i) => (
          <li key={b.n}>
            <div
              className={cn(
                "flex items-baseline gap-3 rounded-full px-3 py-2 transition-[background-color,color] duration-300 md:rounded-2xl",
                i === step ? "bg-fg/10 text-fg" : "text-muted",
              )}
            >
...
export function HowSection() {
  const { ref, step } = useChapterBeat(HOW_BEATS.length);
  return (
    <section id="how" ref={ref} className="chapter scroll-mt-24">
      <div className="mx-auto grid max-w-6xl md:grid-cols-[minmax(0,240px)_1fr] lg:grid-cols-[minmax(0,280px)_1fr]">
        <Rail kicker="How it works" title="Name. Match. Split." beats={HOW_BEATS} step={step} />
        <div className="min-w-0 px-4 sm:px-6">
          <Scene>
            <p className="text-[10px] font-medium uppercase tracking-[0.22em] text-muted">Beat 01</p>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-3">
              <Door icon={Keyboard} label="Type" detail="Title and artist in one field. Slash focuses the box." />
              <Door icon={Mic} label="Speak" detail="Tap the mic, say the song, tap again. It becomes text." />
              <Door icon={Pointer} label="Tap" detail="The atlas chips below are real recordings, already mapped." />
            </div>
          </Scene>

          <Scene>
            <p className="text-[10px] font-medium uppercase tracking-[0.22em] text-muted">Beat 02</p>
            <h3 className="mt-3 font-display text-3xl font-semibold tracking-tight sm:text-5xl">
              Match the recording,
              <br />
              not a mood board.
            ...
            <p className="mt-4 max-w-xl text-base leading-relaxed text-muted">
              Public catalogs return artwork, year, and a store tag. That tag is a shelf label — useful
              for shops, too wide to be a lineage.
            </p>
            <div className="mt-8 grid gap-4 sm:grid-cols-[9rem_1fr]">
              ...
              <article className="glass rounded-[28px] p-6">
                <div className="text-[10px] font-medium uppercase tracking-[0.2em] text-muted">Catalog hit</div>
                <div className="mt-3 font-display text-3xl font-semibold tracking-tight">Lean On</div>
          ...
          <Scene>
            <p className="text-[10px] font-medium uppercase tracking-[0.22em] text-muted">Beat 03</p>
            <h3 className="mt-3 font-display text-3xl font-semibold tracking-tight sm:text-5xl">
            ...
            <ol className="mt-8 grid gap-3">
              <li className="glass grid gap-2 rounded-[28px] p-5 sm:grid-cols-[7.5rem_1fr_auto] sm:items-center">
                <span className="text-[10px] font-medium uppercase tracking-[0.2em] text-muted">Genre</span>
                <span className="font-display text-2xl font-semibold">EDM</span>
                <span className="text-sm text-muted">the festival family</span>
              </li>
              <li className="glass grid gap-2 rounded-[28px] p-5 sm:grid-cols-[7.5rem_1fr_auto] sm:items-center">
                <span className="text-[10px] font-medium uppercase tracking-[0.2em] text-muted">Subgenre</span>
              ...
              <li className="glass grid gap-2 rounded-[28px] p-5 sm:grid-cols-[7.5rem_1fr_auto] sm:items-center">
                <span className="text-[10px] font-medium uppercase tracking-[0.2em] text-muted">Microgenre</span>
...
export function LineageSection() {
  const { ref, step } = useChapterBeat(LINEAGE_BEATS.length);
  return (
      ...
      <div className="mx-auto grid max-w-6xl md:grid-cols-[minmax(0,240px)_1fr] lg:grid-cols-[minmax(0,280px)_1fr]">
        ...
        <div className="min-w-0 px-4 sm:px-6">
          <Scene>
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              <article className="glass rounded-[28px] p-6 opacity-60">
                <div className="text-[10px] font-medium uppercase tracking-[0.2em] text-subtle">Rejected</div>
              ...
              <article className="glass-strong rounded-[28px] p-6">
                <div className="text-[10px] font-medium uppercase tracking-[0.2em] text-accent">Kept</div>
          ...
          <Scene>
            ...
            <p className="mt-4 max-w-xl text-base leading-relaxed text-muted">
              Tech house, deep house, tropical house — same family, different nights. Lean On sits in
              tropical house, then moombahton. That last step is the microgenre.
            </p>
            <div className="mt-8">
              <div className="text-[10px] font-medium uppercase tracking-[0.2em] text-muted">EDM → House</div>
          ...
          <Scene>
            ...
            <p className="mt-4 max-w-xl text-base leading-relaxed text-muted">
              Every row is three different words. Read down a column and you get family, room, scene.
            </p>
            <div className="mt-8 overflow-hidden rounded-[28px] glass">
              ...
              {WORKED.map((row) => (
                ...
                >
                  <div>
                    <div className="font-display text-lg font-semibold tracking-tight">{row.title}</div>
...
export function AtlasSection({
  ...
  return (
    <section id="atlas" className="mx-auto min-h-dvh max-w-6xl scroll-mt-24 px-4 py-32 sm:px-6">
      <p className="reveal text-[10px] font-medium uppercase tracking-[0.22em] text-muted">Atlas</p>
      <h2 className="reveal mt-4 font-display text-4xl font-semibold tracking-tight sm:text-6xl">
        Try a known recording
      </h2>
      <p className="reveal mt-5 max-w-2xl text-base leading-relaxed text-muted">
      ...
      <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {EXAMPLES.map((ex, i) => (
          <button
            ...
            className={cn(
              "reveal glass group min-h-36 rounded-[32px] p-7 text-left",
          ...
          >
            ...
            <div className="mt-6 font-display text-2xl font-semibold tracking-tight">{ex.label}</div>
...
export function AboutSection() {
  return (
    <section id="about" className="mx-auto flex min-h-[70vh] max-w-6xl flex-col justify-center px-4 py-24 sm:px-6">
      <p className="reveal text-[10px] font-medium uppercase tracking-[0.22em] text-muted">About</p>
      <h2 className="reveal mt-4 font-display text-4xl font-semibold tracking-tight sm:text-6xl">
